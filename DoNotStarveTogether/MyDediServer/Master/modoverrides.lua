return {
	["workshop-347079953"] = { enabled = true }, --食物/药品信息显示.
	["workshop-351325790"] = { enabled = true }, --按网格摆放.
	["workshop-356930882"] = { enabled = true }, --无限帐篷使用.
	["workshop-358749986"] = { enabled = true }, --强化位置指示.
	["workshop-362175979"] = { enabled = true }, --颜色指示虫洞.
	["workshop-362191380"] = { enabled = true }, --高ping战士保护器.
	["workshop-365119238"] = { enabled = true }, --智能烹饪锅.
	--["workshop-367546858"] = { enabled = true }, --中文语言包.
	--["workshop-374550642"] = { enabled = true }, --改变物品堆叠上限.
	["workshop-375850593"] = { enabled = true }, --增加一个背包和一个防具槽.
	["workshop-375859599"] = { enabled = true }, --显示怪物血量.
	["workshop-376333686"] = { enabled = true }, --显示血、饥饿、脑残、温度、季节、月相和天数.
	["workshop-378160973"] = { enabled = true }, --显示玩家位置.
	["workshop-441957490"] = { enabled = true }, --人物Wise Wolf.
	["workshop-458587300"] = { enabled = true }, --告示牌传送.
	["workshop-447092740"] = { enabled = true }, --更多动作.
	["workshop-572538624"] = { enabled = true }, --中文增强.
	["workshop-623749604"] = { enabled = true }, --可以建立大容量地窖.
	["workshop-666723837"] = { enabled = true }, --角色道士温宇.
	["workshop-831523966"] = { enabled = true }, --999堆叠.
	["workshop-1207269058"] = { enabled = true }, --简单的血条显示 \n附带伤害显示
}
